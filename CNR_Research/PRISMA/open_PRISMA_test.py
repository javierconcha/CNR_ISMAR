...
def stack_rgb(cube,r_wl,g_wl,b_wl):
    ...
    stackedRGB = np.stack((red.transpose(),green.transpose(),blue.transpose()),axis=2)
...
def plot_geo(img,lats,lons,lat_plot_limits=[-90,90],lon_plot_limits=[-180,180],one_channel=False):
	# solution taken from: 
    ...
    if one_channel:
        m.pcolormesh(lons, lats, img, latlon=True)
    else:
        mesh_rgb = img
...
#%%
plt.figure()
data = vnir[:,3,:].astype(float)/ScaleFactor_vnir-Offset_vnir
lats = lat_vnir[:,:]
lons = lon_vnir[:,:]
...
g_wl = 550
b_wl = 460
cube = vnir[:].astype(float)/ScaleFactor_vnir-Offset_vnir
stackedRGB = stack_rgb(cube,r_wl,g_wl,b_wl)
# plt.figure()
...
sp1_vnir = vnir[sp1_y,3:,sp1_x].astype(float)
sp1_vnir = sp1_vnir/ScaleFactor_vnir-Offset_vnir
sp1_swir = swir[sp1_y,:-2,sp1_x].astype(float)
sp1_swir = sp1_swir/ScaleFactor_swir-Offset_swir

sp2_vnir = vnir[sp2_y,3:,sp2_x].astype(float)
sp2_vnir = sp2_vnir/ScaleFactor_vnir-Offset_vnir
sp2_swir = swir[sp2_y,:-2,sp2_x].astype(float)
sp2_swir = sp2_swir/ScaleFactor_swir-Offset_swir

sp3_vnir = vnir[sp3_y,3:,sp3_x].astype(float)
sp3_vnir = sp3_vnir/ScaleFactor_vnir-Offset_vnir
sp3_swir = swir[sp3_y,:-2,sp3_x].astype(float)
sp3_swir = sp3_swir/ScaleFactor_swir-Offset_swir

sp4_vnir = vnir[sp4_y,3:,sp4_x].astype(float)
sp4_vnir = sp4_vnir/ScaleFactor_vnir-Offset_vnir
sp4_swir = swir[sp4_y,:-2,sp4_x].astype(float)
sp4_swir = sp4_swir/ScaleFactor_swir-Offset_swir
...
# get geolocation info ----
lat_swir = f['/HDFEOS/SWATHS/PRS_L2D_HCO/Geolocation Fields/Latitude']
lon_swir = f['/HDFEOS/SWATHS/PRS_L2D_HCO/Geolocation Fields/Longitude']

lat_vnir = f['/HDFEOS/SWATHS/PRS_L2D_HCO/Geolocation Fields/Latitude']
lon_vnir = f['/HDFEOS/SWATHS/PRS_L2D_HCO/Geolocation Fields/Longitude']
...
# :L2ScalePanMax = 1.0f; // float
# :L2ScalePanMin = 0.0f; // float
# :L2ScaleSwirMax = 0.958448f; // float
# :L2ScaleSwirMin = 0.0f; // float
# :L2ScaleVnirMax = 0.285101f; // float
# :L2ScaleVnirMin = 0.0f; // float
...
data = vnir[:,band_ind,:].astype(float)/ScaleFactor_vnir-Offset_vnir
lats = lat_vnir[:,:]
lons = lon_vnir[:,:]
...
# get geolocation info ----
lat_swir = f['/HDFEOS/SWATHS/PRS_L2C_HCO/Geolocation Fields/Latitude']
lon_swir = f['/HDFEOS/SWATHS/PRS_L2C_HCO/Geolocation Fields/Longitude']

lat_vnir = f['/HDFEOS/SWATHS/PRS_L2C_HCO/Geolocation Fields/Latitude']
lon_vnir = f['/HDFEOS/SWATHS/PRS_L2C_HCO/Geolocation Fields/Longitude']
...
plot_geo(data.transpose(),lats.transpose(),lons.transpose(),lat_plot_limits=[37,47],lon_plot_limits=[5,20],one_channel=True)
...
plt.ylabel('Reflectance (%)')    
#%%
plot_geo(stackedRGB[:-1,:-1,:],lats.transpose(),lons.transpose(),lat_plot_limits=[37,47],lon_plot_limits=[5,20],one_channel=False)
